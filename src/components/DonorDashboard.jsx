...
// Import shared dashboard CSS styles
import './Dashboard.css';
// Import notification bell component for showing alerts
import NotificationBell from './NotificationBell.jsx';
// Import message center component for communication
import MessageCenter from './MessageCenter.jsx';
// Import reusable stat card component for displaying metrics
import StatCard from './StatCard.jsx';
// Import utility functions to get default data for demo purposes
import { getDefaultDonations, getDefaultDonorNotifications, getDefaultDonorMessages } from '../utils/defaultData.js';

// DonorDashboard component - main interface for food donors
// Receives user object and logout handler as props
const DonorDashboard = ({ user, onLogout }) => {
  // State to store list of donations made by this donor
  const [donations, setDonations] = useState([]);
  // State to control visibility of "Add Donation" modal
  const [showModal, setShowModal] = useState(false);
  // State to store notifications for this donor
  const [notifications, setNotifications] = useState([]);
  // State to store messages for this donor
  const [messages, setMessages] = useState([]);
  // State to store calculated statistics about donor's impact
  const [stats, setStats] = useState({
    totalDonations: 0,    // Total number of donations made
    totalWeight: 0,       // Total weight of food donated (in kg)
    peopleHelped: 0,      // Estimated number of people helped
    co2Saved: 0          // Estimated CO2 emissions saved (in kg)
  });
  // State to store form data for new donation entry
  const [formData, setFormData] = useState({
    foodType: '',      // Type of food being donated
    quantity: '',      // Amount of food
    unit: 'kg',        // Unit of measurement (kg, g, lbs)
    expiryDate: '',    // When the food expires
    location: '',      // Pickup location
    description: ''    // Additional details about the donation
  });
  // State to store form validation errors
  const [errors, setErrors] = useState({});

  // Effect hook to load user data when component mounts or user changes
  useEffect(() => {
    // Try to load existing donations from browser's localStorage
    // Using user-specific key to separate data for different users
    ...
    if (storedDonations) {
      try {
        ...
        // Set donations in component state
        setDonations(parsed);
        // Calculate and update statistics based on loaded donations
      ...
      // First-time user - load demo data to show how the dashboard looks
